...
const CartSection =({no,name,avgRating,ratingCount,costForTwo,isVeg,discription,imageUrl,handleAdd}) =>{
	return(
		<div className="d-flex justify-content-between container align-item-center p-4 gap-2 h-[200px]">
			...
			<div className="relative h-full w-[20%]" >
				...
				<button className="bg-white rounded-xl absolute bottom-[-20px] left-[70px] bottom-0.5 text-lg text-green-700 font-bold px-4 py-2 drop-shadow " onClick={handleAdd}>ADD</button>
			</div>
		</div>
		
		);
		console.log(CartSection.length)
};
